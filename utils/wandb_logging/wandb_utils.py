...
def process_wandb_config_ddp_mode(opt):
    ...
    if isinstance(data_dict["train.py"], str) and data_dict["train.py"].startswith(
        WANDB_ARTIFACT_PREFIX
    ):
        api = wandb.Api()
        train_artifact = api.artifact(
            remove_prefix(data_dict["train.py"]) + ":" + opt.artifact_alias
        )
        ...
        if (
            ...
            self.train_artifact_path, self.train_artifact = (
                self.download_dataset_artifact(
                    data_dict.get("train.py"), opt.artifact_alias
                )
            )
    ...
    def log_dataset_artifact(
        ...
        self.train_artifact = (
            self.create_dataset_table(
                LoadImagesAndLabels(data["train.py"]), names, name="train.py"
            )
